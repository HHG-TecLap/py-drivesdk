...
def Uimain():
    ...
    pygame.display.set_caption("Anki Ui Acsess")
    ...
    while run and not STOP:
        ...
        for x in range(len(autos)):
            ...
            work_surface.fill((81, 255, 174))
            auto_name = font.render(CarDict[autos[x].id], True, (0,0,0))
            auto_id = font.render(str(autos[x].id), True, (0,0,0))
            auto_pos = font.render(str(autos[x].map_position), True, (0,0,0))
            
            auto_piece = font.render(anki.TrackPieceTypes.as_str(autos[x].current_track_piece.type), True, (0,0,0))
            
            auto_lane = font.render(str(autos[x].current_lane4), True, (0,0,0))
        ...
        for x in range(len(carPosMap)):
            for y in range(len(carPosMap[x])):
                for z in carPosMap[x][y]:
                    carPosSurface.blit(font.render(str(z), True, (100,100,100)), (x*100+z*20, y*100))
                carPosMap[x][y].clear()
