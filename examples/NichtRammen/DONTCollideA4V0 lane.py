...
async def Main():
    ...
    carOnMap : list[shiftRegister.shiftRegister, bool] = [[]]
    for i in range(len(control.map)):
        carOnMap.append([shiftRegister.shiftRegister(), True])
    ...
    def registering(vehicle : anki.Vehicle, ID):
        def regIntern ():
            ...
            carOnMap[vehicle.map_position][0].addNew(ID)
            if carOnMap[vehicle.map_position-1][0].read() == ID:
                carOnMap[vehicle.map_position - 1][0].remove()
        return regIntern
        ...
        while True:
            ...
            if (intersect2.read() not in (0,id)) & (vehicle.map_position in(3,11)):
                ...
                while intersect2.read() not in (0,id):
                    await asyncio.sleep(0)
                await vehicle.setSpeed(speed)
            if (carOnMap[vehicle.map_position][0].read() not in (0,id)):
                await vehicle.stop()
                while carOnMap[vehicle.map_position][0].read() not in (0,id):
                    await asyncio.sleep(0)
                await vehicle.setSpeed(speed)
            await asyncio.sleep(0)
